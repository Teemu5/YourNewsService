...
async function headlessAddStorage(context, storageRequest) {
    ...
    if (storageRequest.serviceConfiguration.serviceName === provider_constants_1.ServiceName.S3) {
        if (s3_walkthrough_1.resourceAlreadyExists()) {
...
async function headlessImportStorage(context, storageRequest) {
    ...
    if (storageRequest.serviceConfiguration.serviceName === provider_constants_1.ServiceName.S3) {
        ...
        await import_s3_1.updateStateFiles(context, questionParameters, answers, true);
    }
    else if (storageRequest.serviceConfiguration.serviceName === provider_constants_1.ServiceName.DynamoDB) {
...
async function createS3StorageArtifacts(context, storageRequest) {
    const storageInput = s3_headless_adapter_1.buildS3UserInputFromHeadlessStorageRequest(context, storageRequest);
    const s3UserInput = await s3_resource_api_1.s3CreateStorageResource(context, storageInput);
    const allowUnauthenticatedIdentities = storageInput.guestAccess && storageInput.guestAccess.length > 0;
    await s3_auth_api_1.checkStorageAuthenticationRequirements(context, s3UserInput.resourceName, allowUnauthenticatedIdentities);
    const lambdaConfig = storageRequest.serviceConfiguration.lambdaTrigger;
    if (lambdaConfig) {
        if (lambdaConfig.mode === 'new') {
            const storageLambdaParams = s3_headless_adapter_1.buildTriggerFunctionParams(lambdaConfig.name);
            await s3_resource_api_1.s3AddStorageLambdaTrigger(context, storageInput.resourceName, storageLambdaParams);
        }
    }
}
async function updateS3StorageArtifacts(context, updateStorageRequest, _storageResource) {
    const lambdaConfig = updateStorageRequest.serviceModification.lambdaTrigger;
    const storageInput = await s3_headless_adapter_1.buildS3UserInputFromHeadlessUpdateStorageRequest(context, updateStorageRequest);
    const allowUnauthenticatedIdentities = storageInput.guestAccess && storageInput.guestAccess.length > 0;
    await s3_auth_api_1.checkStorageAuthenticationRequirements(context, storageInput.resourceName, allowUnauthenticatedIdentities);
    let s3UserInput = await s3_resource_api_1.s3UpdateUserInput(context, storageInput);
    if (lambdaConfig) {
        if (lambdaConfig.mode === 'new') {
            const storageLambdaParams = s3_headless_adapter_1.buildTriggerFunctionParams(lambdaConfig.name);
            s3UserInput = await s3_resource_api_1.s3AddStorageLambdaTrigger(context, storageInput.resourceName, storageLambdaParams);
        }
    }
    return s3UserInput;
}
