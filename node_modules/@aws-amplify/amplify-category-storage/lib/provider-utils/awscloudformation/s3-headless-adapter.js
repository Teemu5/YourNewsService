...
function buildS3UserInputFromHeadlessStorageRequest(context, storageRequest) {
    ...
    const defaultS3UserInput = s3_defaults_1.getAllDefaults(context.amplify.getProjectDetails(), shortId);
    let s3UserInput = {
...
async function buildS3UserInputFromHeadlessUpdateStorageRequest(context, storageRequest) {
    ...
    let s3UserInputs = await __1.s3GetUserInput(context, resourceName);
    if (permissions) {
...
function buildTriggerFunctionParams(triggerFunctionName) {
    const storageLambdaParams = {
        ...
        triggerPrefix: [{ prefix: 'protected/', prefixTransform: s3_user_input_types_1.S3TriggerPrefixTransform.ATTACH_REGION },
