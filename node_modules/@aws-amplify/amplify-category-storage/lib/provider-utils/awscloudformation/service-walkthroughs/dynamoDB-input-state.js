...
class DynamoDBInputState {
    ...
    isCLIInputsValid(cliInputs) {
        if (!cliInputs) {
            cliInputs = this.getCliInputPayload();
        }
        const schemaValidator = new amplify_cli_core_1.CLIInputSchemaValidator(this._service, this._category, 'DynamoDBCLIInputs');
        schemaValidator.validateInput(JSON.stringify(cliInputs));
    }
    saveCliInputPayload(cliInputs) {
    ...
    migrate() {
        ...
        const partitionKey = {
            ...
            fieldType: cfn_template_utils_1.getFieldType(oldParameters.partitionKeyType),
        };
        let sortKey;
        if (oldParameters.sortKeyName) {
            sortKey = {
                fieldName: oldParameters.sortKeyName,
                fieldType: cfn_template_utils_1.getFieldType(oldParameters.sortKeyType),
            };
        }
        ...
        const getType = (attrList, attrName) => {
            ...
            attrList.forEach((attr) => {
                if (attr.AttributeName === attrName) {
                    attrType = cfn_template_utils_1.getFieldType(attr.AttributeType);
                }
            });
            return attrType;
        };
