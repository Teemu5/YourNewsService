...
class S3InputState {
    constructor(resourceName, userInput) {
    ...
    inferAuthPermissions(oldParams) {
        if (oldParams.selectedAuthenticatedPermissions && ((oldParams.s3PermissionsAuthenticatedPublic && oldParams.s3PermissionsAuthenticatedPublic != 'DISALLOW') ||
    ...
    inferGuestPermissions(oldParams) {
        if (oldParams.selectedGuestPermissions && ((oldParams.s3PermissionsGuestPublic && oldParams.s3PermissionsGuestPublic != 'DISALLOW') ||
    ...
    genInputParametersForMigration(oldS3Params) {
        ...
        let userInputs = {
            ...
            policyUUID: s3_walkthrough_1.buildShortUUID(),
    ...
    async migrate(context) {
        ...
        try {
            const authMigrationAccepted = await s3_auth_api_1.migrateAuthDependencyResource(context);
    ...
    async isCLIInputsValid(cliInputs) {
        ...
        const schemaValidator = new amplify_cli_core_1.CLIInputSchemaValidator(this._service, this._category, 'S3UserInputs');
