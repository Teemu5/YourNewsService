...
async function addWalkthrough(context, defaultValuesFilename, serviceMetadata, options) {
    ...
    try {
        const authMigerationAccepted = await s3_auth_api_1.migrateAuthDependencyResource(context);
        if (!authMigerationAccepted) {
            amplify_cli_core_1.exitOnNextTick(0);
        }
    }
    catch (error) {
        await s3_errors_1.printErrorAuthResourceMigrationFailed(context);
        amplify_cli_core_1.exitOnNextTick(0);
    }
    await s3_questions_1.askAndInvokeAuthWorkflow(context);
    const resourceName = await getS3ResourceNameFromMeta(amplifyMeta);
    if (resourceName) {
        await s3_errors_1.printErrorAlreadyCreated(context);
        amplify_cli_core_1.exitOnNextTick(0);
    }
    else {
        const policyID = buildShortUUID();
        const defaultValues = s3_defaults_1.getAllDefaults(amplify.getProjectDetails(), policyID);
        const storageResourceName = await s3_questions_1.askResourceNameQuestion(context, defaultValues);
        const bucketName = await s3_questions_1.askBucketNameQuestion(context, defaultValues, storageResourceName);
        ...
        if (userPoolGroupList && userPoolGroupList.length > 0) {
            cliInputs = await s3_questions_1.askGroupOrIndividualAccessFlow(userPoolGroupList, context, cliInputs);
        }
        else {
            cliInputs.storageAccess = await await s3_questions_1.askWhoHasAccessQuestion(context, defaultValues);
            cliInputs.authAccess = await s3_questions_1.askAuthPermissionQuestion(context, defaultValues);
            cliInputs.guestAccess = await await s3_questions_1.conditionallyAskGuestPermissionQuestion(cliInputs.storageAccess, context, defaultValues);
        }
        ...
        await s3_auth_api_1.checkStorageAuthenticationRequirements(context, storageResourceName, allowUnauthenticatedIdentities);
        const cliInputsState = new s3_user_input_state_1.S3InputState(cliInputs.resourceName, cliInputs);
...
async function updateWalkthrough(context) {
    ...
    if (storageResourceName === undefined) {
        await s3_errors_1.printErrorNoResourcesToUpdate(context);
        amplify_cli_core_1.exitOnNextTick(0);
    }
    else {
        ...
        let cliInputsState = new s3_user_input_state_1.S3InputState(storageResourceName, undefined);
        ...
        if (!cliInputsState.cliInputFileExists()) {
            if (headlessMigrate ||
                (await amplify_prompts_1.prompter.yesOrNo(amplify_cli_core_1.getMigrateResourceMessageForOverride(amplify_cli_core_1.AmplifyCategories.STORAGE, storageResourceName), true))) {
        ...
        if (userPoolGroupList && userPoolGroupList.length > 0) {
            ...
            cliInputs.storageAccess = await s3_questions_1.askWhoHasAccessQuestion(context, previousUserInput);
            cliInputs.authAccess = await s3_questions_1.askAuthPermissionQuestion(context, previousUserInput);
            cliInputs.guestAccess = await s3_questions_1.conditionallyAskGuestPermissionQuestion(cliInputs.storageAccess, context, previousUserInput);
        }
...
async function migrateStorageCategory(context, resourceName) {
    let cliInputsState = new s3_user_input_state_1.S3InputState(resourceName, undefined);
    if (!cliInputsState.cliInputFileExists()) {
        await cliInputsState.migrate(context);
...
async function startUpdateTriggerFunctionFlow(context, resourceName, policyID, existingTriggerFunction) {
    ...
    do {
        const triggerOperationAnswer = await s3_questions_1.askUpdateTriggerSelection(existingTriggerFunction);
...
function getS3ResourcesFromAmplifyMeta(amplifyMeta) {
    if (!amplifyMeta.hasOwnProperty(amplify_cli_core_1.AmplifyCategories.STORAGE)) {
        return undefined;
    }
    const resources = {};
...
async function getExistingFunctionsForTrigger(context, excludeFunctionName, isInteractive) {
    ...
    const adminTriggerFunction = await s3_resource_api_1.s3GetAdminTriggerFunctionName(context);
...
async function interactiveCreateNewLambdaAndUpdateCFN(context) {
    ...
    await s3_questions_1.askAndOpenFunctionEditor(context, newTriggerFunction);
...
async function interactiveAddExistingLambdaAndUpdateCFN(context, existingTriggerFunction = undefined, existingLambdaResources = undefined) {
    ...
    const selectedFunction = await s3_questions_1.askSelectExistingFunctionToAddTrigger(lambdaResources);
...
async function interactiveAskTriggerTypeFlow(context, _policyID, existingTriggerFunction, existingLambdaResources = undefined) {
    const triggerTypeAnswer = await s3_questions_1.askTriggerFunctionTypeQuestion();
