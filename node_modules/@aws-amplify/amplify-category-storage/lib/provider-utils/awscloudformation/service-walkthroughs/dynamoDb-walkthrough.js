...
async function addWalkthrough(context, defaultValuesFilename) {
    ...
    const cliInputsState = new dynamoDB_input_state_1.DynamoDBInputState(cliInputs.resourceName);
    cliInputsState.saveCliInputPayload(cliInputs);
    const stackGenerator = new ddb_stack_transform_1.DDBStackTransform(cliInputs.resourceName);
...
async function updateWalkthrough(context) {
    ...
    const cliInputsState = new dynamoDB_input_state_1.DynamoDBInputState(resourceName);
    ...
    if (!cliInputsState.cliInputFileExists()) {
        if (headlessMigrate || (await amplify_prompts_1.prompter.yesOrNo(amplify_cli_core_1.getMigrateResourceMessageForOverride(amplify_cli_core_1.AmplifyCategories.STORAGE, resourceName), true))) {
            cliInputsState.migrate();
            const stackGenerator = new ddb_stack_transform_1.DDBStackTransform(resourceName);
            stackGenerator.transform();
        }
        else {
            return;
    ...
    let existingAttributeDefinitions = [];
...
async function askTriggersQuestion(context, resourceName, existingTriggerFunctions) {
    let triggerFunctions = existingTriggerFunctions || [];
...
async function askGSIQuestion(indexableAttributeList, attributeDefinitions, existingGSIList) {
    ...
    if (existingGSIList &&
        ...
        (await amplify_prompts_1.prompter.yesOrNo('Do you want to keep existing global seconday indexes created on your table?', true))) {
...
async function askResourceNameQuestion(defaultValues) {
    const resourceName = await amplify_prompts_1.prompter.input('Provide a friendly name', {
        validate: amplify_prompts_1.alphanumeric(),
