...
async function s3GetUserInput(context, s3ResourceName) {
    if (s3_walkthrough_1.isMigrateStorageRequired(context, s3ResourceName)) {
        await s3_walkthrough_1.migrateStorageCategory(context, s3ResourceName);
    }
    let cliInputsState = new s3_user_input_state_1.S3InputState(s3ResourceName, undefined);
...
async function s3CreateStorageResource(context, storageInput) {
    let storageResourceName = s3GetResourceName();
...
async function s3AddStorageLambdaTrigger(context, s3ResourceName, storageLambdaTrigger) {
    ...
    await s3_walkthrough_1.createNewLambdaAndUpdateCFN(context, s3UserInput.triggerFunction, undefined);
...
async function s3APIHelperTransformAndSaveState(context, storageInput, phase) {
    if (phase != amplify_cli_core_1.CLISubCommandType.ADD && s3_walkthrough_1.isMigrateStorageRequired(context, storageInput.resourceName)) {
        await s3_walkthrough_1.migrateStorageCategory(context, storageInput.resourceName);
    }
    let cliInputsState;
    if (phase === amplify_cli_core_1.CLISubCommandType.ADD) {
        cliInputsState = new s3_user_input_state_1.S3InputState(storageInput.resourceName, storageInput);
    }
    else {
        cliInputsState = new s3_user_input_state_1.S3InputState(storageInput.resourceName, undefined);
    }
