...
const run = async (context) => {
    ...
    if (amplifyMeta[amplify_cli_core_1.AmplifyCategories.STORAGE][selectedResourceName].service === amplify_cli_core_1.AmplifySupportedService.DYNAMODB) {
        const resourceInputState = new dynamoDB_input_state_1.DynamoDBInputState(selectedResourceName);
        if (!resourceInputState.cliInputFileExists()) {
            if (await amplify_prompts_1.prompter.yesOrNo(amplify_cli_core_1.getMigrateResourceMessageForOverride(amplify_cli_core_1.AmplifyCategories.STORAGE, selectedResourceName, false), true)) {
                resourceInputState.migrate();
                const stackGenerator = new ddb_stack_transform_1.DDBStackTransform(selectedResourceName);
        ...
        const s3ResourceInputState = new s3_user_input_state_1.S3InputState(selectedResourceName, undefined);
        if (!s3ResourceInputState.cliInputFileExists()) {
            if (await amplify_prompts_1.prompter.yesOrNo(amplify_cli_core_1.getMigrateResourceMessageForOverride(amplify_cli_core_1.AmplifyCategories.STORAGE, selectedResourceName, false), true)) {
                ...
                stackGenerator.transform(amplify_cli_core_1.CLISubCommandType.MIGRATE);
            }
            else {
                return;
            }
    ...
    await amplify_cli_core_1.generateOverrideSkeleton(context, srcPath, destPath);
};
exports.run = run;
