...
async function s3GetBucketUserInputDefault(project, shortId, accessType) {
    let defaultS3UserInputs = s3_defaults_1.getAllDefaults(project, shortId);
    switch (accessType) {
...
async function console(context) {
    amplify_prompts_1.printer.info(`to be implemented: ${amplify_cli_core_1.AmplifyCategories.STORAGE} console`);
}
...
async function transformCategoryStack(context, resource) {
    if (resource.service === amplify_cli_core_1.AmplifySupportedService.DYNAMODB) {
        if (canResourceBeTransformed(resource.resourceName)) {
            const stackGenerator = new ddb_stack_transform_1.DDBStackTransform(resource.resourceName);
            await stackGenerator.transform();
        }
    }
    else if (resource.service === amplify_cli_core_1.AmplifySupportedService.S3) {
        await s3_stack_transform_1.transformS3ResourceStack(context, resource);
    }
}
exports.transformCategoryStack = transformCategoryStack;
function canResourceBeTransformed(resourceName) {
    const resourceInputState = new dynamoDB_input_state_1.DynamoDBInputState(resourceName);
...
const executeAmplifyHeadlessCommand = async (context, headlessPayload) => {
    switch (context.input.command) {
        case 'add':
            await storage_configuration_helpers_1.headlessAddStorage(context, await amplify_util_headless_input_1.validateAddStorageRequest(headlessPayload));
            break;
        case 'update':
            await storage_configuration_helpers_1.headlessUpdateStorage(context, await amplify_util_headless_input_1.validateUpdateStorageRequest(headlessPayload));
            break;
        case 'remove':
            await storage_configuration_helpers_1.headlessRemoveStorage(context, await amplify_util_headless_input_1.validateRemoveStorageRequest(headlessPayload));
            break;
        case 'import':
            await storage_configuration_helpers_1.headlessImportStorage(context, await amplify_util_headless_input_1.validateImportStorageRequest(headlessPayload));
            break;
        default:
...
async function initEnv(context) {
    ...
    const storageTasks = tasks.map(storageResource => {
        ...
        return async () => {
            const config = await awscloudformation_1.updateConfigOnEnvInit(context, amplify_cli_core_1.AmplifyCategories.STORAGE, resourceName, service);
