...
async function createFrontEndConfigFile(context, exportPath) {
    ...
    const spinner = ora_1.default(`Extracting outputs from ${rootStackName}`);
    spinner.start();
    const providerPlugin = context.amplify.getProviderPlugins(context);
    const providers = Object.keys(providerPlugin);
    try {
        ...
        const validatedExportPath = amplify_cli_core_1.validateExportDirectoryPath(exportPath, amplify_cli_core_1.PathConstants.DefaultFrontEndExportFolder);
        await frontendHandlerModule.createFrontendConfigsAtPath(context, get_resource_outputs_1.getResourceOutputs(meta), get_resource_outputs_1.getResourceOutputs(cloudMeta), validatedExportPath);
