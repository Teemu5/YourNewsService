...
async function showStatusTable(tableViewFilter) {
    ...
    const { resourcesToBeCreated, resourcesToBeUpdated, resourcesToBeDeleted, resourcesToBeSynced, rootStackUpdated, allResources, tagsUpdated, } = await resource_status_data_1.getMultiCategoryStatus(tableViewFilter);
    if (amplifyProjectInitStatus === get_cloud_init_status_1.CLOUD_INITIALIZED) {
        resource_status_view_1.viewEnvInfo();
    }
    resource_status_view_1.viewSummaryTable({ resourcesToBeUpdated, resourcesToBeCreated, resourcesToBeDeleted, resourcesToBeSynced, allResources });
    if (tagsUpdated) {
        print_1.print.info('\nTag Changes Detected');
    }
    if (tableViewFilter.verbose) {
        await resource_status_view_1.viewResourceDiffs({ resourcesToBeUpdated, resourcesToBeDeleted, resourcesToBeCreated });
    }
...
async function showResourceTable(category, resourceName, filteredResources) {
    const amplifyProjectInitStatus = get_cloud_init_status_1.getCloudInitStatus();
    const { resourcesToBeCreated, resourcesToBeUpdated, resourcesToBeDeleted, resourcesToBeSynced, allResources, tagsUpdated, rootStackUpdated, } = await resource_status_data_1.getResourceStatus(category, resourceName, undefined, filteredResources);
    if (amplifyProjectInitStatus === get_cloud_init_status_1.CLOUD_INITIALIZED) {
        resource_status_view_1.viewEnvInfo();
    }
    resource_status_view_1.viewSummaryTable({ resourcesToBeUpdated, resourcesToBeCreated, resourcesToBeDeleted, resourcesToBeSynced, allResources });
