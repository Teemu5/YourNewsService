...
function resourceToTableRow(resource, operation) {
    return [resource_status_diff_1.capitalize(resource.category), resource.resourceName, operation, resource.providerPlugin];
}
...
async function getResourceStatus(category, resourceName, providerName, filteredResources) {
    ...
    const rootStackUpdated = await root_stack_status_1.isRootStackModifiedSinceLastPush(root_stack_status_1.getHashForRootStack);
    return {
        ...
        await asyncForEach(Object.keys(categoryItem), async (resource) => {
            ...
            if (currentAmplifyMeta[categoryName] &&
                ...
                if (categoryName === 'function' && currentAmplifyMeta[categoryName][resource].service === "LambdaLayer") {
                    ...
                    const backendModified = await isBackendDirModifiedSinceLastPush(resource, categoryName, (_b = currentAmplifyMeta[categoryName][resource]) === null || _b === void 0 ? void 0 : _b.lastPushTimeStamp, getHashForResourceDir);
                    if (backendModified) {
                        amplifyMeta[categoryName][resource].resourceName = resource;
                        amplifyMeta[categoryName][resource].category = categoryName;
                        resources.push(amplifyMeta[categoryName][resource]);
