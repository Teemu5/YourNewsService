...
const run = async (context) => {
    ...
    const servicesMetadata = supported_services_1.getSupportedServices();
    ...
    if (lodash_1.default.isEmpty(existingAuth)) {
        return amplify_prompts_1.printer.warn('Project does not contain auth resources. Add auth using `amplify add auth`.');
    }
    else {
        ...
        for (const authResourceName of authResources) {
            ...
            else if (serviceMeta.service === amplify_cli_core_1.AmplifySupportedService.COGNITO && serviceMeta.serviceType === 'imported') {
                amplify_prompts_1.printer.error('Updating imported Auth resource is not supported.');
                return context;
            }
            else if (serviceMeta.service === amplify_cli_core_1.AmplifySupportedService.COGNITO && !amplify_cli_core_1.FeatureFlags.getBoolean('auth.forceAliasAttributes')) {
    ...
    await check_for_auth_migration_1.checkAuthResourceMigration(context, resourceName, true);
    ...
    try {
        const result = await amplify.serviceSelectionPrompt(context, __1.category, supported_services_1.getSupportedServices());
        const options = {
