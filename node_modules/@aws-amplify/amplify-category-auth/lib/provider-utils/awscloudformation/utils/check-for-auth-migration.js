...
const checkAuthResourceMigration = async (context, authName, isUpdate) => {
    ...
    if (!imported) {
        const cliState = new auth_input_state_1.AuthInputState(authName);
        if (!cliState.cliInputFileExists()) {
            ...
            if (headlessMigrate ||
                (await amplify_prompts_1.prompter.yesOrNo(amplify_cli_core_1.getMigrateResourceMessageForOverride(amplify_cli_core_1.AmplifyCategories.AUTH, authName, isUpdate), true))) {
                await migrate_override_resource_1.migrateResourceToSupportOverride(authName);
                const cliInputs = cliState.getCLIInputPayload();
                await generate_auth_stack_template_1.generateAuthStackTemplate(context, cliInputs.cognitoConfig.resourceName);
                return true;
            }
            return false;
        }
