...
async function buildCustomResources(context, resourceName) {
    const spinner = ora_1.default('Building custom resources');
    try {
        spinner.start();
        ...
        amplify_prompts_1.printer.error('There was an error building the custom resources');
...
async function generateDependentResourcesType(context) {
    ...
    const allResources = dependency_management_utils_1.getAllResources();
    const params = {
...
async function buildResource(context, resource) {
    ...
    const packageManager = amplify_cli_core_1.getPackageManager(targetDir);
    ...
    await generate_cfn_from_cdk_1.generateCloudFormationFromCDK(resource.resourceName);
}
