...
async function processDockerConfig(context, resource, srcPath, askForExposedContainer = false) {
    ...
    if ((await shouldUpdateSecrets(context, secrets)) || isInitialDeploy) {
        ...
        const { cfnTemplate } = amplify_cli_core_1.readCFNTemplate(path.join(amplify_cli_core_1.pathManager.getBackendDirPath(), category_constants_1.category, resourceName, exports.cfnFileName(resourceName)));
