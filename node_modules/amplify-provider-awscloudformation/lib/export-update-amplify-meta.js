...
async function run(context, stackName) {
    ...
    if (!rootStack) {
        amplify_prompts_1.printer.error(`${stackName} could not be found, are you sure you are using the right credentials?`);
        throw new amplify_cli_core_1.ExportedStackNotFoundError(`${stackName} not found`);
    }
    if (rootStack.StackStatus !== 'UPDATE_COMPLETE' && rootStack.StackStatus !== 'CREATE_COMPLETE') {
        throw new amplify_cli_core_1.ExportedStackNotInValidStateError(`${stackName} not in UPDATE_COMPLETE or CREATE_COMPLETE state`);
    }
