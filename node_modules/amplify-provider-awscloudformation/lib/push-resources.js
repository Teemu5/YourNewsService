...
exports.generateAndUploadRootStack = exports.formNestedStack = exports.uploadTemplateToS3 = exports.getCfnFiles = exports.storeCurrentCloudBackend = exports.updateStackForAPIMigration = exports.run = exports.rootStackFileName = exports.defaultRootStackFileName = void 0;
...
async function run(context, resourceDefinition, rebuild = false) {
    var _a, _b, _c, _d;
    ...
    try {
        ...
        for (const resource of resources.filter(r => r.category === constants_1.FunctionCategoryName && r.service === constants_1.FunctionServiceNameLambdaLayer)) {
        ...
        await build_override_enabled_resources_1.buildOverridesEnabledResources(context);
        ...
        if (amplify_cli_core_1.FeatureFlags.getBoolean('graphQLTransformer.enableIterativeGSIUpdates')) {
            const gqlResource = utils_1.getGqlUpdatedResource(rebuild ? resources : resourcesToBeUpdated);
            if (gqlResource) {
                const gqlManager = await graphql_transformer_1.GraphQLResourceManager.createInstance(context, gqlResource, cloudformationMeta.StackId, rebuild);
                deploymentSteps = await gqlManager.run();
                const modelsBeingReplaced = gqlManager.getTablesBeingReplaced().map(meta => meta.stackName);
                deploymentSteps = await disconnect_dependent_resources_1.prependDeploymentStepsToDisconnectFunctionsFromReplacedModelTables(context, modelsBeingReplaced, deploymentSteps);
        ...
        await template_description_utils_1.prePushTemplateDescriptionHandler(context, resourcesToBeCreated);
        ...
        if (resourcesToBeCreated.length > 0 ||
            ...
            if (deploymentSteps.length > 1) {
                ...
                await generateAndUploadRootStack(context, rootStackFilepath, exports.rootStackFileName);
                const finalStep = {
                ...
                disconnect_dependent_resources_1.postDeploymentCleanup(s3, cloudformationMeta.DeploymentBucketName);
            }
            else {
                spinner.start();
                ...
                try {
                    ...
                    await initializer_1.storeRootStackTemplate(context, nestedStack);
...
exports.run = run;
async function canAutoResolveGraphQLAuthError(message) {
...
async function prepareResource(context, resource) {
    ...
    if (resource.service === constants_1.FunctionServiceNameLambdaLayer) {
        ...
        const { cfnTemplate } = amplify_cli_core_1.readCFNTemplate(cfnFilePath);
...
async function updateCloudFormationNestedStack(context, nestedStack, resourcesToBeCreated, resourcesToBeUpdated) {
    ...
    const rootStackFilePath = path.join(amplify_cli_core_1.pathManager.getRootStackBuildDirPath(projectRoot), exports.rootStackFileName);
    await initializer_1.storeRootStackTemplate(context, nestedStack);
    const transformedStackPath = await cfn_pre_processor_1.preProcessCFNTemplate(rootStackFilePath);
...
function getCfnFiles(category, resourceName, options) {
    const backEndDir = amplify_cli_core_1.pathManager.getBackendDirPath();
    const resourceDir = path.normalize(path.join(backEndDir, category, resourceName));
...
async function updateS3Templates(context, resourcesToBeUpdated, amplifyMeta) {
    ...
    for (const { category, resourceName, service } of resourcesToBeUpdated) {
        ...
        for (const cfnFile of cfnFiles) {
            await cfn_pre_processor_1.writeCustomPoliciesToCFNTemplate(resourceName, service, cfnFile, category);
...
async function formNestedStack(context, projectDetails, categoryName, resourceName, serviceName, skipEnv) {
    var _a;
    let rootStack;
    rootStack = await override_manager_1.transformRootStack(context);
    ...
    if (Object.keys(metaToBeUpdated).length) {
        ...
        lodash_1.default.set(teamProviderInfo, [envName, 'awscloudformation'], tpiResourceParams);
        amplify_cli_core_1.stateManager.setTeamProviderInfo(projectPath, teamProviderInfo);
    }
    try {
