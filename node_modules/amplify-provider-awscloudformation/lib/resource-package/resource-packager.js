...
class ResourcePackager {
    ...
    async filterResourcesToBeDeployed(deploymentResources) {
        ...
        if (apiResourceTobeUpdated.length) {
            const functionResourceToBeUpdated = await remove_dependent_function_1.ensureValidFunctionModelDependencies(this.context, apiResourceTobeUpdated, deploymentResources.allResources);
    ...
    async preBuildResources(resources) {
        ...
        for await (const lambdaLayerResource of this.filterResourceByCategoryService(resources, FUNCTION_CATEGORY.NAME, FUNCTION_CATEGORY.SERVICE.LAMBDA_LAYER)) {
            await lambdaLayerInvocations_1.legacyLayerMigration(this.context, lambdaLayerResource.resourceName);
        }
        await lambdaLayerInvocations_1.prePushLambdaLayerPrompt(this.context, resources);
        return resources;
    }
    ...
    async packageResources(builtResources) {
        const { FUNCTION_CATEGORY } = constants_1.Constants;
        return await Promise.all(builtResources.map(async (resource) => {
            if (!resource.build) {
                return resource;
            }
    ...
    async postPackageResource(pacakgedResources) {
        ...
        if (this.resourcesHasCategoryService(pacakgedResources, API_CATEGORY.NAME, API_CATEGORY.SERVICE.APP_SYNC)) {
            await transform_graphql_schema_1.transformGraphQLSchema(this.context, {
                handleMigration: opts => push_resources_1.updateStackForAPIMigration(this.context, 'api', undefined, opts),
    ...
    storeS3BucketInfo(packagedResource, bucketName) {
        ...
        lodash_1.default.set(this.amplifyTeamProviderInfo, [this.envInfo.envName, CATEGORIES, packagedResource.category, packagedResource.resourceName], s3Info);
        lodash_1.default.set(this.amplifyMeta, [packagedResource.category, packagedResource.resourceName, S3_BUCKET], s3Info);
    }
    async generateCategoryCloudFormation(resources) {
        if (this.resourcesHasApiGatewaysButNotAdminQueries(resources)) {
            ...
            consolidate_apigw_policies_1.consolidateApiGatewayPolicies(this.context, stackName);
        }
        await auth_transform_1.prePushAuthTransform(this.context, resources);
        ...
        switch (resource.category) {
            case API_CATEGORY.NAME:
                if (resource.service === API_CATEGORY.SERVICE.ELASTIC_CONTAINER) {
                    ...
                    lodash_1.default.set(this.amplifyMeta, [resource.category, resource.resourceName, EXPOSED_CONTAINER], exposedContainer);
                }
                break;
    ...
    async postGenerateCategoryCloudFormation(resources) {
        ...
        await template_description_utils_1.prePushTemplateDescriptionHandler(this.context, resources);
        for await (const resource of resources) {
            ...
            for await (const cfnFile of cfnFiles) {
                if (resource.build &&
                    ...
                    const { cfnTemplate, templateFormat } = amplify_cli_core_1.readCFNTemplate(cfnFile);
                    ...
                    await amplify_cli_core_1.writeCFNTemplate(cfnTemplate, cfnFile, { templateFormat });
                }
                const transformedCFNPath = await cfn_pre_processor_1.preProcessCFNTemplate(cfnFile);
                await cfn_pre_processor_1.writeCustomPoliciesToCFNTemplate(resource.resourceName, resource.service, cfnFile, resource.category);
    ...
    getCfnTemplatePathsForResource(resource) {
        const { cfnFiles } = push_resources_1.getCfnFiles(resource.category, resource.resourceName, {
            absolute: true,
        });
        return cfnFiles;
    }
    async generateRootStack() {
        return await push_resources_1.formNestedStack(this.context, { amplifyMeta: this.amplifyMeta });
    }
}
