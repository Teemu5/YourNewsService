...
class ResourceExport extends resource_packager_1.ResourcePackager {
    ...
    async downloadLambdaLayerContent(resource) {
        ...
        if (layerVersions.length > 0) {
            ...
            for await (const lambdaLayer of layerVersions) {
                ...
                await zip_util_1.downloadZip(s3instance, exportPath, lambdaLayer.contentPath, this.envInfo.envName);
            }
        }
    }
    ...
    async processAndWriteCfnTemplate(cfnTemplate, destinationPath, templateFormat, deleteParameters) {
        ...
        await amplify_cli_core_1.writeCFNTemplate(template, destinationPath, { templateFormat });
        return parameters;
    }
    ...
    async writeCategoryCloudFormation(resources) {
        ...
        for await (const resource of resources) {
            ...
            for (const cfnFile of resource.transformedCfnPaths) {
                ...
                lodash_1.default.set(this.amplifyMeta, [resource.category, resource.resourceName, PROVIDER_METADATA], {
        ...
        if (this.resourcesHasContainers(resources)) {
            const template = (await env_level_constructs_1.getNetworkResourceCfn(this.context, stackName));
            ...
            lodash_1.default.set(this.amplifyMeta, [PROVIDER, PROVIDER_NAME, NETWORK_STACK_S3_URL], this.createTemplateUrl(bucket, NETWORK_STACK_FILENAME));
        }
        if (this.resourcesHasApiGatewaysButNotAdminQueries(resources)) {
            ...
            if (fs.existsSync(apiGWAuthFile)) {
                ...
                lodash_1.default.set(this.amplifyMeta, [PROVIDER, PROVIDER_NAME, API_GATEWAY_AUTH_URL], this.createTemplateUrl(bucket, APIGW_AUTH_STACK_FILE_NAME, API_CATEGORY.NAME));
            }
        }
        ...
        if (amplify_cli_core_1.FeatureFlags.getBoolean('auth.breakCircularDependency') && authResource) {
            ...
            if (fs.existsSync(pathToTriggerFile)) {
                ...
                lodash_1.default.set(this.amplifyMeta, [PROVIDER, PROVIDER_NAME, AUTH_TRIGGER_TEMPLATE_URL], this.createTemplateUrl(bucket, AUTH_TRIGGER_TEMPLATE_FILE, AUTH_CATEGORY.NAME));
            }
        }
        return stackParameters;
    }
    ...
    async modifyRootStack(template, deleteParameters) {
        ...
        template.Description = template_description_utils_1.getDefaultTemplateDescription(this.context, 'root');
        return template;
    }
