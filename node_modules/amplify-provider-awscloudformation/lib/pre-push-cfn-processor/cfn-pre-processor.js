...
async function preProcessCFNTemplate(filePath) {
    const { templateFormat, cfnTemplate } = amplify_cli_core_1.readCFNTemplate(filePath);
...
async function writeCustomPoliciesToCFNTemplate(resourceName, service, cfnFile, category) {
    ...
    const { templateFormat, cfnTemplate } = amplify_cli_core_1.readCFNTemplate(cfnPath);
    const newCfnTemplate = amplify_cli_core_1.generateCustomPoliciesInTemplate(cfnTemplate, resourceName, service, category);
    await amplify_cli_core_1.writeCFNTemplate(newCfnTemplate, cfnPath, { templateFormat });
}
