...
exports.getTransformerVersion = exports.getDirectiveDefinitions = exports.transformGraphQLSchema = exports.searchablePushChecks = void 0;
...
function searchablePushChecks(context, map, apiName) {
    ...
    if (searchableModelTypes.length) {
        ...
        const instanceType = lodash_1.default.get(teamProviderInfo, [currEnv, 'categories', 'api', apiName, graphql_transformer_common_1.ResourceConstants.PARAMETERS.ElasticsearchInstanceType], 't2.small.elasticsearch');
        if (instanceType === 't2.small.elasticsearch' || instanceType === 't3.small.elasticsearch') {
            amplify_prompts_1.printer.warn(`Your instance type for OpenSearch is ${instanceType}, you may experience performance issues or data loss. Consider reconfiguring with the instructions here https://docs.amplify.aws/cli/graphql-transformer/searchable/`);
        }
    }
}
...
async function transformGraphQLSchema(context, options) {
    ...
    searchablePushChecks(context, directiveMap.types, parameters[graphql_transformer_common_1.ResourceConstants.PARAMETERS.AppSyncApiName]);
    ...
    const sanityCheckRulesList = graphql_transformer_core_1.getSanityCheckRules(isNewAppSyncAPI, ff, allowDestructiveUpdates);
    const buildConfig = {
        ...options,
        buildParameters,
        projectDirectory: resourceDir,
...
async function getDirectiveDefinitions(context, resourceDir) {
    const transformerVersion = getTransformerVersion(context);
    if (transformerVersion === 2) {
        return transform_graphql_schema_1.getDirectiveDefinitions(context, resourceDir);
    }
...
async function getBucketName(context, s3ResourceName) {
    ...
    const bucketParameters = amplify_cli_core_1.stateManager.getResourceParametersJson(undefined, 'storage', s3ResourceName);
...
function getTransformerVersion(context) {
    ...
    const transformerVersion = amplify_cli_core_2.FeatureFlags.getNumber('graphQLTransformer.transformerVersion');
...
async function migrateToTransformerVersionFeatureFlag(context) {
    ...
    if (useExperimentalPipelineTransformer && transformerVersion === 1) {
        ...
        await amplify_cli_core_2.FeatureFlags.reloadValues();
