...
const sandbox_mode_helpers_1 = require("../utils/sandbox-mode-helpers");
const amplify_prompts_1 = require("amplify-prompts");
const sanity_check_1 = require("./sanity-check");
const auth_mode_compare_1 = require("../utils/auth-mode-compare");
...
⚠️  WARNING: Some types do not have authorization rules configured. That means all create, read, update, and delete operations are denied on these types:`, 'yellow');
        ...
        amplify_prompts_1.printer.info('Learn more about "@auth" authorization rules here: https://docs.amplify.aws/cli/graphql-transformer/auth\n', 'yellow');
    }
}
...
async function transformGraphQLSchema(context, options) {
    ...
    const adminRoles = await utils_1.getAdminRoles(context, resourceName);
    const identityPoolId = await utils_1.getIdentityPoolId(context);
    ...
    const lastDeployedProjectConfig = fs_extra_1.default.existsSync(previouslyDeployedBackendDir)
        ...
        : undefined;
    const sandboxModeEnabled = sandbox_mode_helpers_1.schemaHasSandboxModeEnabled(project.schema);
    ...
    if (showSandboxModeMessage)
        sandbox_mode_helpers_1.showGlobalSandboxModeWarning();
    else
        warnOnAuth(directiveMap.types);
    transform_graphql_schema_1.searchablePushChecks(context, directiveMap.types, parameters[graphql_transformer_common_1.ResourceConstants.PARAMETERS.AppSyncApiName]);
    const transformerListFactory = getTransformerFactory(context, resourceDir);
    if (sandboxModeEnabled && options.promptApiKeyCreation) {
        const apiKeyConfig = await sandbox_mode_helpers_1.showSandboxModePrompts(context);
        if (apiKeyConfig)
    ...
    const allowDestructiveUpdates = ((_b = (_a = context === null || context === void 0 ? void 0 : context.input) === null || _a === void 0 ? void 0 : _a.options) === null || _b === void 0 ? void 0 : _b[constants_1.destructiveUpdatesFlag]) || ((_d = (_c = context === null || context === void 0 ? void 0 : context.input) === null || _c === void 0 ? void 0 : _c.options) === null || _d === void 0 ? void 0 : _d.force);
    const sanityCheckRules = graphql_transformer_core_2.getSanityCheckRules(isNewAppSyncAPI, ff, allowDestructiveUpdates);
    const buildConfig = {
        ...options,
        buildParameters,
...
place .graphql files in a directory at ${schemaDirPath}`);
    if (auth_mode_compare_1.isAuthModeUpdated(options)) {
...
async function buildAPIProject(opts) {
    ...
    const currCloudLocation = opts.currentCloudBackendDirectory ? path_1.default.join(opts.currentCloudBackendDirectory, 'build') : undefined;
    if (opts.projectDirectory && !opts.dryRun) {
        await utils_1.writeDeploymentToDisk(builtProject, buildLocation, opts.rootStackFileName, opts.buildParameters, opts.minify);
        const sanityChecker = new sanity_check_1.GraphQLSanityCheck(opts.sanityCheckRules, opts.rootStackFileName, currCloudLocation, buildLocation);
...
async function _buildProject(opts) {
    ...
    const userDefinedSlots = {
        ...user_defined_slots_1.parseUserDefinedSlots(userProjectConfig.pipelineFunctions),
        ...user_defined_slots_1.parseUserDefinedSlots(userProjectConfig.resolvers),
    };
    ...
    return utils_1.mergeUserConfigWithTransformOutput(userProjectConfig, transformOutput, opts);
}
