...
const prependDeploymentStepsToDisconnectFunctionsFromReplacedModelTables = async (context, modelsBeingReplaced, deploymentSteps) => {
    ...
    functionsDependentOnReplacedModelTables = await utils_1.getDependentFunctions(modelsBeingReplaced, allFunctionNames, getFunctionParamsSupplier(context));
    const { deploymentSteps: disconnectFuncsSteps, lastMetaKey } = await utils_1.generateIterativeFuncDeploymentSteps(new aws_sdk_1.CloudFormation(await configuration_manager_1.loadConfiguration(context)), rootStackId, functionsDependentOnReplacedModelTables);
    await utils_1.generateTempFuncCFNTemplates(functionsDependentOnReplacedModelTables);
    await utils_1.uploadTempFuncDeploymentFiles(await aws_s3_1.S3.getInstance(context), functionsDependentOnReplacedModelTables);
    return utils_1.prependDeploymentSteps(disconnectFuncsSteps, deploymentSteps, lastMetaKey);
};
...
const postDeploymentCleanup = async (s3Client, deploymentBucketName) => {
    ...
    await Promise.all(functionsDependentOnReplacedModelTables.map(funcName => fs.remove(utils_1.localPrefix(funcName))));
};
exports.postDeploymentCleanup = postDeploymentCleanup;
const getFunctionParamsSupplier = (context) => async (functionName) => {
