...
const logger = aws_logger_1.fileLogger('disconnect-dependent-resources');
...
const generateTempFuncCFNTemplates = async (dependentFunctions) => {
    ...
    for (const funcName of dependentFunctions) {
        const { cfnTemplate, templateFormat } = amplify_cli_core_1.readCFNTemplate(path.join(amplify_cli_core_1.pathManager.getResourceDirectoryPath(undefined, 'function', funcName), `${funcName}-cloudformation-template.json`));
        replaceFnImport(cfnTemplate);
        const tempPath = getTempFuncTemplateLocalPath(funcName);
        await amplify_cli_core_1.writeCFNTemplate(cfnTemplate, tempPath, { templateFormat });
...
const generateIterativeFuncDeploymentOp = async (cfnClient, rootStackId, functionName) => {
    ...
    const { parameters, capabilities } = await amplify_resource_state_utils_1.getPreviousDeploymentRecord(cfnClient, funcStackId);
    ...
    const tpiCfnParams = ((_c = (_b = (_a = tpi === null || tpi === void 0 ? void 0 : tpi[env]) === null || _a === void 0 ? void 0 : _a.categories) === null || _b === void 0 ? void 0 : _b.function) === null || _c === void 0 ? void 0 : _c[functionName]) || {};
...
const getTempFuncTemplateLocalPath = (funcName) => path.join(exports.localPrefix(funcName), tempTemplateFilename(funcName));
const getTempFuncMetaLocalPath = (funcName) => path.join(exports.localPrefix(funcName), tempMetaFilename(funcName));
