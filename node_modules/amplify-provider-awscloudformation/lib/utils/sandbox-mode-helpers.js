...
async function showSandboxModePrompts(context) {
    if (!api_key_helpers_1.hasApiKey()) {
...
function showGlobalSandboxModeWarning() {
    amplify_prompts_1.printer.info(`
...
function schemaHasSandboxModeEnabled(schema) {
    const { definitions } = graphql_1.parse(schema);
    ...
    if (authScalarMatch && defaultValueNameMatch && defaultValueValueMatch) {
        ...
        throw Error('There was a problem with your auth configuration. Learn more about auth here: https://docs.amplify.aws/cli/graphql-transformer/auth');
    }
}
