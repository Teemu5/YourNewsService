...
const lambdaEventSourceHandler = (resourceName, resource, cfnContext) => {
    const batchSize = field_parser_1.parseValue(resource.Properties.BatchSize, cfnContext);
    const eventSourceArn = field_parser_1.parseValue(resource.Properties.EventSourceArn, cfnContext);
    const functionName = field_parser_1.parseValue(resource.Properties.FunctionName, cfnContext);
    const startingPosition = field_parser_1.parseValue(resource.Properties.StartingPosition, cfnContext);
    return {
