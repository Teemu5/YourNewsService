...
class StorageTest {
    ...
    async trigger(context) {
        ...
        this.storageSimulator.getServer.on('event', async (eventObj) => {
            const meta = context.amplify.getProjectDetails().amplifyMeta;
            const existingStorage = meta.storage;
            let backendPath = context.amplify.pathManager.getBackendDirPath();
            const resourceName = Object.keys(existingStorage)[0];
            ...
            const config = await load_lambda_config_1.loadLambdaConfig(context, triggerName);
            const invoker = await amplify_category_function_1.getInvoker(context, { handler: config.handler, resourceName: triggerName, envVars: config.environment });
            try {
    ...
    createLocalStorage(context, resourceName) {
        const directoryPath = path.join(utils_1.getMockDataDirectory(context), 'S3');
