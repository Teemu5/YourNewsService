...
class CLIInputSchemaGenerator {
    constructor(typeDefs) {
        this.TYPES_SRC_ROOT = path_1.default.join('.', 'src', 'provider-utils', 'awscloudformation', 'service-walkthrough-types');
        this.SCHEMA_FILES_ROOT = path_1.default.join('.', 'resources', 'schemas');
    ...
    getSvcFileAbsolutePath(normalizedSvcName) {
        return path_1.default.resolve(this.getTypesSrcRootForSvc(normalizedSvcName));
    }
    getTypesSrcRootForSvc(normalizedSvcName) {
        return path_1.default.join(this.TYPES_SRC_ROOT, `${normalizedSvcName}-user-input-types.ts`);
    }
    normalizeServiceToFilePrefix(svcName) {
    ...
    generateJSONSchemas() {
        ...
        for (const typeDef of this.serviceTypeDefs) {
            ...
            const program = typescript_json_schema_1.getProgramFromFiles([svcAbsoluteFilePath]);
            const schemaGenerator = typescript_json_schema_1.buildGenerator(program, settings);
            const typeSchema = schemaGenerator === null || schemaGenerator === void 0 ? void 0 : schemaGenerator.getSchemaForSymbol(typeDef.typeName);
            const outputSchemaFilePath = path_1.default.resolve(path_1.default.join(this.SCHEMA_FILES_ROOT, normalizedServiceName, this.getSchemaFileNameForType(typeDef.typeName)));
            if (!force && fs_extra_1.default.existsSync(outputSchemaFilePath)) {
                this.printWarningSchemaFileExists();
                return generatedFilePaths;
            }
            fs_extra_1.default.ensureFileSync(outputSchemaFilePath);
            fs_extra_1.default.writeFileSync(outputSchemaFilePath, JSON.stringify(typeSchema, undefined, 4));
...
class CLIInputSchemaValidator {
    constructor(service, category, schemaFileName) {
    ...
    async getUserInputSchema() {
        try {
            return await Promise.resolve().then(() => __importStar(require(generateSchemaPath(this._category, this._service, this._schemaFileName))));
        }
        catch (ex) {
...
const generateSchemaPath = (category, service, schemaFileName) => {
    return path_1.default.join(`@aws-amplify/amplify-category-${category}`, 'resources', 'schemas', `${service}`, `${schemaFileName}.schema.json`);
};
