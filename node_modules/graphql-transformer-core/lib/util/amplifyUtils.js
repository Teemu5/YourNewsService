...
async function buildProject(opts) {
    ...
    if (opts.projectDirectory && !opts.dryRun) {
        ...
        await sanity_check_1.sanityCheckProject(lastBuildPath, thisBuildPath, opts.rootStackFileName, opts.sanityCheckRules.diffRules, opts.sanityCheckRules.projectRules);
    }
    return builtProject;
}
    ...
    if (!isNewAppSyncAPI) {
        ...
        if (iterativeUpdatesEnabled) {
            if (!allowDestructiveUpdates) {
                diffRules.push(sanity_check_1.getCantEditKeySchemaRule(iterativeUpdatesEnabled), sanity_check_1.getCantAddLSILaterRule(iterativeUpdatesEnabled), sanity_check_1.getCantRemoveLSILater(iterativeUpdatesEnabled), sanity_check_1.getCantEditLSIKeySchemaRule(iterativeUpdatesEnabled), sanity_check_1.cantRemoveTableAfterCreation);
            }
            projectRules.push(sanity_check_1.cantHaveMoreThan500ResourcesRule);
        }
        else {
            diffRules.push(sanity_check_1.getCantEditKeySchemaRule(), sanity_check_1.getCantAddLSILaterRule(), sanity_check_1.getCantRemoveLSILater(), sanity_check_1.getCantEditLSIKeySchemaRule(), sanity_check_1.cantEditGSIKeySchemaRule, sanity_check_1.cantAddAndRemoveGSIAtSameTimeRule);
            if (!allowDestructiveUpdates) {
