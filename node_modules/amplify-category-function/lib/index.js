...
async function initEnv(context) {
    ...
    allResources
        ...
        if (s3Bucket) {
            const tpiResourceParams = lodash_1.default.get(teamProviderInfo, [envName, 'categories', constants_1.categoryName, resourceName], {});
            lodash_1.default.assign(tpiResourceParams, s3Bucket);
            lodash_1.default.set(teamProviderInfo, [envName, 'categories', constants_1.categoryName, resourceName], tpiResourceParams);
            lodash_1.default.set(amplifyMeta, [constants_1.categoryName, resourceName, 's3Bucket'], s3Bucket);
        }
    });
    resourcesToBeCreated.forEach(resource => {
        ...
        if (service === "Lambda") {
            if (sourceEnv && isNewEnv) {
                const groupName = lodash_1.default.get(teamProviderInfo, [sourceEnv, 'categories', constants_1.categoryName, resourceName, 'GROUP']);
                if (groupName) {
                    lodash_1.default.set(teamProviderInfo, [envName, 'categories', constants_1.categoryName, resourceName, 'GROUP'], groupName);
                }
            }
        }
    });
    ...
    if (isNewEnv) {
        ...
        await environmentVariablesHelper_1.askEnvironmentVariableCarryOut(context, sourceEnv, context.exeInfo.localEnvInfo.projectPath, yesFlagSet);
        await cloneSecretsOnEnvInitHandler_1.cloneSecretsOnEnvInitHandler(context, sourceEnv, envName);
    }
}
...
function getBuilder(context, resourceName, buildType) {
    ...
    return async () => {
        await buildFunction_1.buildFunction(context, { resourceName, buildType, lastBuildTimestamp, lastBuildType });
    };
}
...
async function handleAmplifyEvent(context, args) {
    ...
    switch (args.event) {
        case 'PrePush':
            await prePushHandler_1.prePushHandler(context);
            break;
        case 'PostPush':
            await postPushHandler_1.postPushHandler(context);
            break;
        case 'InternalOnlyPostEnvRemove':
            await postEnvRemoveHandler_1.postEnvRemoveHandler(context, (_a = args === null || args === void 0 ? void 0 : args.data) === null || _a === void 0 ? void 0 : _a.envName);
            break;
    }
}
exports.handleAmplifyEvent = handleAmplifyEvent;
async function lambdaLayerPrompt(context, resources) {
    const lambdaLayerResource = getLambdaLayerResources(resources);
    await packageLayer_1.checkContentChanges(context, lambdaLayerResource);
}
exports.lambdaLayerPrompt = lambdaLayerPrompt;
function getLambdaLayerResources(resources) {
    return resources.filter(r => r.service === "LambdaLayer" && r.category === constants_1.categoryName);
}
async function postPushCleanup(resource, envName) {
    const lambdaLayerResource = getLambdaLayerResources(resource);
    lambdaLayerResource.forEach(resource => {
        layerConfiguration_1.deleteLayerVersionsToBeRemovedByCfn(resource.resourceName, envName);
        layerConfiguration_1.deleteLayerVersionPermissionsToBeUpdatedInCfn(resource.resourceName, envName);
    });
}
