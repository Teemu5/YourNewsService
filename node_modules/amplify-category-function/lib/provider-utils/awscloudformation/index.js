...
async function addFunctionResource(context, category, service, serviceConfig, parameters) {
    ...
    await storeResources_1.createFunctionResources(context, completeParams);
    amplify_cli_core_2.createDefaultCustomPoliciesFile(category, completeParams.resourceName);
...
async function updateFunctionResource(context, category, service, parameters, resourceToUpdate) {
    ...
    if (parameters && 'trigger' in parameters) {
        ...
        await storeResources_1.saveMutableState(context, parameters);
        storeResources_1.saveCFNParameters(parameters);
    }
    else {
...
async function updateLayerResource(context, service, serviceConfig, parameters) {
    ...
    await storeResources_1.updateLayerArtifacts(context, updateWalkthroughResult.parameters, {
...
async function updateConfigOnEnvInit(context, resourceName, service) {
    ...
    if (service === "Lambda") {
        ...
        if (Array.isArray(resourceParams.lambdaLayers) && resourceParams.lambdaLayers.length) {
            ...
            modifiedLambdaLayers.push(...convertLayersTypes_1.convertProjectLayersToExternalLayers(resourceParams.lambdaLayers, envName));
            modifiedLambdaLayers.push(...convertLayersTypes_1.convertExternalLayersToProjectLayers(resourceParams.lambdaLayers, envName));
    ...
    else if (layerHelpers_1.isMultiEnvLayer(resourceName) && service === "LambdaLayer") {
        ...
        if (currentCloudVersionHash) {
            lodash_1.default.set(amplifyMeta, [constants_1.categoryName, resourceName, constants_2.versionHash], currentCloudVersionHash);
        }
        if (context.input.command === 'env' && ((_a = context.input) === null || _a === void 0 ? void 0 : _a.subCommands.includes('checkout')) && !((_c = (_b = context.exeInfo) === null || _b === void 0 ? void 0 : _b.inputParams) === null || _c === void 0 ? void 0 : _c.restore)) {
            ...
            if (currentParametersJson) {
                const backendParametersJson = layerConfiguration_1.loadLayerParametersJson(resourceName);
            ...
            const { cfnTemplate: currentCfnTemplate } = amplify_cli_core_1.readCFNTemplate(currentCfnTemplatePath, { throwIfNotExist: false }) || {};
            if (currentCfnTemplate !== undefined) {
                await amplify_cli_core_1.writeCFNTemplate(currentCfnTemplate, amplify_cli_core_1.pathManager.getResourceCfnTemplatePath(projectPath, constants_1.categoryName, resourceName));
            }
        }
    }
}
