...
class LayerCloudState {
    ...
    async loadLayerDataFromCloud(context, layerName) {
        var _a;
        const spinner = ora_1.default('Loading layer data from the cloud...').start();
        try {
            ...
            const layerVersionList = await lambdaClient.listLayerVersions(layerHelpers_1.isMultiEnvLayer(layerName) ? `${layerName}-${envName}` : layerName);
            const cfnClient = await providerPlugin.getCloudFormationSdk(context);
            const stackList = await cfnClient.listStackResources();
            const layerStacks = (_a = stackList === null || stackList === void 0 ? void 0 : stackList.StackResourceSummaries) === null || _a === void 0 ? void 0 : _a.filter(stack => stack.LogicalResourceId.includes(layerName) && stack.ResourceType === 'AWS::CloudFormation::Stack');
