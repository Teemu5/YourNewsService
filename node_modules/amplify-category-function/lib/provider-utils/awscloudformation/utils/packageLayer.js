...
const packageLayer = async (context, resource, isExport) => {
    var _a;
    const previousHash = layerHelpers_1.loadPreviousLayerHash(resource.resourceName);
    const currentHash = await layerHelpers_1.ensureLayerVersion(context, resource.resourceName, previousHash);
    ...
    const { runtimes } = layerConfiguration_1.loadLayerConfigurationFile(resource.resourceName);
    const distDir = path.join(resourcePath, 'dist');
    fs.ensureDirSync(distDir);
    const destination = path.join(distDir, 'latest-build.zip');
    let layerSizeInBytes = 0;
    layerSizeInBytes += await amplify_cli_core_1.getFolderSize([path.join(resourcePath, 'lib'), path.join(resourcePath, 'opt')]);
    if (layerSizeInBytes > constants_1.lambdaPackageLimitInMB * 1024 ** 2) {
        throw new Error(`Lambda layer ${resource.resourceName} is too large: ${amplify_cli_core_1.convertNumBytes(layerSizeInBytes).toMB()}/${constants_1.lambdaPackageLimitInMB} MB`);
    }
    ...
    await zipResource_1.zipPackage(zipEntries, destination);
...
async function checkContentChanges(context, layerResources) {
    var _a;
    const changedLayerResources = await layerHelpers_1.getChangedResources(layerResources);
    ...
    if (changedLayerResources.length > 0) {
        context.print.info('');
        if (layerResources.filter(layer => layerHelpers_1.loadPreviousLayerHash(layer.resourceName) !== undefined).length > 0) {
        ...
        const prepushNotificationMessage = changedLayerResources.map(layer => {
            ...
            const parameters = layerHelpers_1.loadStoredLayerParameters(context, resourceName);
        ...
        for (const layer of changedLayerResources) {
            ...
            if (!accepted) {
                ...
                context.print.info('');
                parameters = await lambdaLayerWalkthrough_1.lambdaLayerNewVersionWalkthrough(parameters, timestampString);
            }
            else {
                parameters.description = `Updated layer version ${timestampString}`;
            }
            await storeResources_1.updateLayerArtifacts(context, parameters, {
                updateMeta: false,
